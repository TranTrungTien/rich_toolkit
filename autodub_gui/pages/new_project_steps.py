...
class VideoStep(_StepPanel):
    ...
    SOURCES: typing.ClassVar[tuple[tuple[str, str], ...]] = (
...
class TranslateStep(_StepPanel):
    ...
    def __init__(self, parent: QWidget | None = None):
        ...
        self.auto_translate.setToolTip(
            "Bật: ứng dụng tự dịch bằng AI. Tắt: ứng dụng dừng ở bước dịch "
            "và hướng dẫn bạn dịch tay.")
        ...
        self.metadata.setToolTip(
            "AI viết sẵn tiêu đề, mô tả và thẻ cho mạng xã hội, lưu vào "
...
class RunStep(_StepPanel):
    ...
    def __init__(self, parent: QWidget | None = None):
        ...
        note = QLabel(
            "Giá của video được tính dựa trên số câu thoại. Nếu dùng DeepSeek, "
            "chi phí sẽ được trừ trực tiếp vào tài khoản DeepSeek của bạn thông "
            "qua API Key.")
        note.setWordWrap(True)
        note.setStyleSheet(
            f"color: {tokens.TEXT_MUTED}; font-size: {tokens.FS_META}px; "
...
class ExportSummaryStep(_StepPanel):
    ...
    def __init__(self, parent: QWidget | None = None):
        ...
        self.notice = QLabel(
            "Video đã sẵn sàng. Bạn có thể xem bản dịch hoặc âm thanh trong "
            "thư mục dự án sau khi xuất.")
    ...
    def set_stats(self, sentences: int, duration_s: float,
        ...
        """Đổ bảng tổng kết: thời lượng, số câu thoại."""
        rows = [
