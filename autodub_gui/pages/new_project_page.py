...
class NewProjectPage(BasePage):
    ...
    def _base_style(self, preset: str) -> dict:
        ...
        except Exception:  # noqa: BLE001, S110 — cấu hình hỏng thì dùng bộ sẵn
            pass
    ...
    def _show_pending(self, result: DubResult) -> None:
        try:
            ...
            provider = Settings.load().translate_provider.lower()
            if provider == "voxdub":
                tip = ("Mẹo: điền API Key DeepSeek hoặc Gemini trong Cài đặt để "
                       "ứng dụng tự dịch, khỏi làm tay.")
            else:
                tip = ("Mẹo: kiểm tra lại API Key hoặc số dư tài khoản của "
                       "nhà cung cấp AI trong Cài đặt.")
