...
def merge_segments(
    ...
    if background_path and os.path.exists(background_path):
        ...
        if ok:
            with wave.open(bg_tmp, "rb") as bg_wave:
                ch = bg_wave.getnchannels()
                return _merge_segments_impl(
                    segments, segment_dir, output_path, total_duration,
                    bg_wave, ch, background_path, background_gain_db,
                    duck_voice_db, bg_tmp
                )
    ...
    return _merge_segments_impl(
        segments, segment_dir, output_path, total_duration,
        None, 1, background_path, background_gain_db,
        duck_voice_db, bg_tmp
    )


def _merge_segments_impl(
    segments: list[dict],
    segment_dir: str,
    output_path: str,
    total_duration: float,
    bg_wave: wave.Wave_read | None,
    ch: int,
