...
class DubPipeline:
    ...
    def _stop_for_export(self, state: dict, work_dir: str) -> DubResult:
        ...
        except Exception:  # noqa: BLE001, S110 — mạng chập chờn thì bỏ qua
            pass
    ...
    def _auto_translate(
        ...
        try:
            ...
            # Analysis pass - supported by all providers if implemented
            if settings.translate_analysis:
                if provider == "voxdub":
                    ...
                    from autodub.text.translate_deepseek import (
                        analyze_transcript,
                        apply_analysis,
                    )
                else:
                    analyze_transcript = apply_analysis = None

                if analyze_transcript:
                    cache = (
                        data_path(work_dir, "video_context.json")
                        if work_dir
                        else None
                    )
            ...
            logger.error(f"Không kết nối được máy chủ dịch: {e}")
            ...
            logger.warning(
                f"Automatic translation failed ({e}); switching to manual translation"
            )
            return None

        # Lượt rà soát — soát câu tràn khung / lẫn chữ Hán / sót ý rồi dịch
        # lại đúng các câu đó. Hỏng thì giữ nguyên bản lượt đầu.
        if settings.translate_review:
            try:
                if provider == "voxdub":
                    from autodub.text.translate_review import review_translations
                    result = review_translations(
                        result, target, source_lang, effective, run_id=run_id
                    )
                elif provider == "deepseek":
                    from autodub.text.translate_deepseek import review_translations
                    result = review_translations(
                        result, target, source_lang, effective
                    )
            except PipelineCancelled:
                raise
            except Exception as e:  # noqa: BLE001
