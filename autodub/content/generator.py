...
def generate_social_metadata(script_original: str, script_translated: str,
                             settings=None, video_title: str = "", job_id: str = "") -> dict:
    """Nhờ máy chủ hoặc AI provider viết tiêu đề, mô tả và hashtag.
    ...
    provider = getattr(settings, "translate_provider", "voxdub").lower()
    ...
    try:
        if provider == "voxdub":
            ...
            from autodub.text.translate_deepseek import generate_post
            metadata = generate_post(
                script_original, script_translated, settings,
                video_title=video_title)
        else:
            logger.info(f"Provider {provider} does not support metadata generation yet")
            return {}
...
def generate_content(
    ...
    result["metadata"] = generate_social_metadata(
        script_original, script_translated, settings=settings,
        video_title=video_title, job_id=job_id)
