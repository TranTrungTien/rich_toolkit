...
def _align_worker_task(item, model, text_field):
...
def align_segments(
    ...
    worker_fn = functools.partial(_align_worker_task, model=model, text_field=text_field)

    if n_workers > 1:
        with ThreadPoolExecutor(max_workers=n_workers) as pool:
            done = list(pool.map(worker_fn, todo))
    else:
        done = [worker_fn(item) for item in todo]
