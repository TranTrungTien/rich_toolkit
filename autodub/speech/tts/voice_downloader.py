...
def ensure_voices_available(settings: Settings, progress_callback=None) -> bool:
    ...
    # Nếu file đã có sẵn (vd: kèm theo repo hoặc đã tải trước đó)
    # thì không cần tải lại, chỉ cần chuyển sang bước enroll.
    have_files = os.path.isfile(manifest)
    if have_files:
        logger.info(f"Đã thấy file manifest tại {manifest}, bỏ qua bước tải")
    else:
        try:
            # 1. Tải voices.zip
            if progress_callback:
            ...
            # Nếu lỗi 404 hoặc mạng mà vẫn chưa có file thì mới fail hẳn.
            # Nếu đã có file thì âm thầm chạy tiếp.
            if not os.path.isfile(manifest):
                raise
            logger.warning(f"Lỗi khi tải voice library ({e}), nhưng đã có file sẵn — tiếp tục")
    ...
    try:
        ...
        logger.exception("Lỗi khi cài đặt voice library")
        if progress_callback:
